...
def process_bill_ocr(request: HttpRequest) -> HttpResponse:
    ...
    POST /api/ocr/process-bill/ with multipart form data.

    Request params:
      - bill_image (file): PNG, JPG, JPEG ≤ 10MB
      - currency (string, optional): INR, USD, EUR, GBP, JPY

    Returns JSON with extracted data: merchant, date, amount, currency, image_base64
    """
    from django.http import JsonResponse
    from .services.bill_ocr_extractor import BillOCRExtractor
    ...
    if "bill_image" not in request.FILES:
        return JsonResponse({"success": False, "error": "bill_image file required"}, status=400)
    ...
    try:
        ...
        # Get currency from request (default: INR)
        currency = request.POST.get("currency", "INR").upper()
        if currency not in {"INR", "USD", "EUR", "GBP", "JPY"}:
            currency = "INR"

        # Extract data using OCR
        bill_data = BillOCRExtractor.extract_from_image(file_bytes, currency=currency)
